...
d3.queue()
	...
	.await((err, mapData, emData) => {
		...
		var selYearLabel = d3.select("#selYearLabel").text("Year: " + selYear);
		...
		d3.selectAll("input[name='Emissions']").on("change", () => {
			d3.event.preventDefault();
...
function resizeCountryChart() {
	...
	let country_width = d3
		.select("#countryChartContainer")
		.node()
	...
	let xAxis = d3.axisBottom(xScale).tickFormat(d3.format(".0f"));
...
function setColors() {
	...
	geoData.forEach(country => {
		filteredData = emmissionsData.filter(
			d => d.year == selYear && d.countryCode == country.id
		);
	...
	d3.selectAll(".country")
		...
		.ease(d3.easeBackIn)
		.attr("fill", d => {
			return d.properties[valueToCheck] !== "NN"
				? scale(d.properties[valueToCheck])
				: "#ccc";
		});
}
...
function drawCountryChart() {
	...
	xScale = d3.scaleLinear().domain(d3.extent(countryData, d => d.year));
